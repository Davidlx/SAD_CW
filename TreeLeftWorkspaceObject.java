...
class TreeLeftWorkspaceObject extends WorkspaceObject
{
   TreeLeft op;
   PartSink sink;

   public TreeLeftWorkspaceObject(Workspace w)
   {
      super(w);
      op = new TreeLeft();
   }
   ...
   {
      out.println("<treeleft>");
      WorkspaceObject c = sink.getContainedPart();
      if(c != null)
	 c.save(out);
      out.println("</treeleft>");
   }
   static TreeLeftWorkspaceObject load(Node node) throws ProgramLoadingException
   {
      if(!node.getNodeName().equals("treeleft"))
	 throw new ProgramLoadingException();

      TreeLeftWorkspaceObject obj = new TreeLeftWorkspaceObject(Workspace.getWorkspace());
