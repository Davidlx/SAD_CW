...
class ListValueWorkspaceObject extends WorkspaceObject
{
   ListValue op;
   PartSink sink;

   public ListValueWorkspaceObject(Workspace w)
   {
      super(w);
      op = new ListValue();
   }
   ...
   JPanel createCustomPanel()
   {
	 ...
	 public void split()
	 {
      ...
      sink.setToolTipText("<html>List to take the first value of.<br>This list must not be empty.</html>");
      p.add(sink);
      return p;
   }
   ...
   static ListValueWorkspaceObject load(Node node) throws ProgramLoadingException
   {
      if(!node.getNodeName().equals("listvalue"))
	 throw new ProgramLoadingException();

      ListValueWorkspaceObject obj = new ListValueWorkspaceObject(Workspace.getWorkspace());
