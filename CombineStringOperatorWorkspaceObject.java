...
class CombineStringOperatorWorkspaceObject extends WorkspaceObject
{
   CombineStringOperator op;
   PartSink leftSink, rightSink;

   public CombineStringOperatorWorkspaceObject(Workspace w)
   {
      super(w);
   }
   ...
   JPanel createCustomPanel()
   {
	 ...
	 public void split()
	 {
      ...
      JLabel eqLab = new JLabel("<html><h3><b>&Dagger;</b></h3></html>", SwingConstants.CENTER);
      p.add(eqLab);
   ...
   {
      return "Combine into String";
   }

   void save(PrintWriter out)
   {
      out.println("<combinestring>");
   ...
   static CombineStringOperatorWorkspaceObject load(Node node) throws ProgramLoadingException
   {
      if(!node.getNodeName().equals("combinestring"))
	 throw new ProgramLoadingException();

      CombineStringOperatorWorkspaceObject obj = new CombineStringOperatorWorkspaceObject(Workspace.getWorkspace());
