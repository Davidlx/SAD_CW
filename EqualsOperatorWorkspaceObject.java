...
class EqualsOperatorWorkspaceObject extends WorkspaceObject
{
   EqualsOperator op;
   PartSink leftSink, rightSink;

   public EqualsOperatorWorkspaceObject(Workspace w)
   {
      super(w);
   }
   ...
   JPanel createCustomPanel()
   {
	 ...
	 public void split()
	 {
      ...
      JLabel eqLab = new JLabel("<html><h3><b>&equiv;</b></h3></html>", SwingConstants.CENTER);
      p.add(eqLab);
   ...
   static EqualsOperatorWorkspaceObject load(Node node) throws ProgramLoadingException
   {
      if(!node.getNodeName().equals("equals"))
	 throw new ProgramLoadingException();

      EqualsOperatorWorkspaceObject obj = new EqualsOperatorWorkspaceObject(Workspace.getWorkspace());
