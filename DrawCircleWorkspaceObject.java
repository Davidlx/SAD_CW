...
class DrawCircleWorkspaceObject extends WorkspaceObject
{
   DrawCircleOperator op;
   PartSink contextSink, xSink, ySink, diamSink;

   public DrawCircleWorkspaceObject(Workspace w)
   {
      super(w);
   }
   ...
   JPanel createCustomPanel()
   {
	 ...
	 public void split()
	 {
      ...
      // diameter operand sink
      diamSink = new PartSink(PartType.EXPRESSION_PART);
      diamSink.addCombineListener(new CombineListener() {
	 public void combined()
	 {
	    WorkspaceObject c = diamSink.getContainedPart();
	    Object o = c.getPart();
	    Expression exp = (Expression)o;
	    op.setDiameterOperand(exp);
	 }
	 public void split()
	 {
	    op.setDiameterOperand(null);
	 }
      });
      diamSink.setToolTipText("Diameter of the circle");
      p.add(diamSink);
   ...
   {
      return "Draw Circle (Graphics)";
   }

   void save(PrintWriter out)
   {
      out.println("<drawcircle>");
      out.println("<context>");
      WorkspaceObject c = contextSink.getContainedPart();
      if(c != null)
	 c.save(out);
	 ...
	 c.save(out);
      out.println("</ycoord>");
      out.println("<diam>");
      c = diamSink.getContainedPart();
      if(c != null)
	 c.save(out);
      out.println("</diam>");
      out.println("</drawcircle>");
   }
   static DrawCircleWorkspaceObject load(Node node) throws ProgramLoadingException
   {
      if(!node.getNodeName().equals("drawcircle"))
	 throw new ProgramLoadingException();

      DrawCircleWorkspaceObject obj = new DrawCircleWorkspaceObject(Workspace.getWorkspace());
      ...
      Node dop = Workspace.getChildElementByName(node, "diam");
	 ...
	 if(ropNode != null)
	 {
      ...
      if(dop != null)
      {
	 Node dopNode = Workspace.getNthChildElement(dop, 0);
	 if(dopNode != null)
	 {
	    WorkspaceObject dopObj = Workspace.dispatchLoad(dopNode);
	    if(dopObj != null)
	    {
	       obj.ySink.progCombine(dopObj);
	    }
	 }
      }
   ...
   public WorkspaceObject getWorkspaceObjectForPart(Object part)  //EEH
   {
	 ...
	 return wo;
      ...
      if(wo != null)
	 return wo;
      wo = diamSink.getContainedPart().getWorkspaceObjectForPart(part);
      if(wo != null)
	 return wo;
