...
class MultiplicationOperatorWorkspaceObject extends WorkspaceObject
{
   MultiplicationOperator op;
   PartSink leftSink, rightSink;

   public MultiplicationOperatorWorkspaceObject(Workspace w)
   {
      super(w);
   }
   ...
   static MultiplicationOperatorWorkspaceObject load(Node node) throws ProgramLoadingException
   {
      if(!node.getNodeName().equals("multiplication"))
	 throw new ProgramLoadingException();

      MultiplicationOperatorWorkspaceObject obj = new MultiplicationOperatorWorkspaceObject(Workspace.getWorkspace());
