...
class ListIsEmptyWorkspaceObject extends WorkspaceObject
{
   ListIsEmpty op;
   PartSink sink;

   public ListIsEmptyWorkspaceObject(Workspace w)
   {
      super(w);
      op = new ListIsEmpty();
   }
   ...
   JPanel createCustomPanel()
   {
	 ...
	 public void split()
	 {
      ...
      sink.setToolTipText("Thie list that we are examining.");
      p.add(sink);
      return p;
   }
   ...
   {
      out.println("<listisempty>");
      WorkspaceObject c = sink.getContainedPart();
      if(c != null)
	 c.save(out);
      out.println("</listisempty>");
   }
   static ListIsEmptyWorkspaceObject load(Node node) throws ProgramLoadingException
   {
      if(!node.getNodeName().equals("listisempty"))
	 throw new ProgramLoadingException();

      ListIsEmptyWorkspaceObject obj = new ListIsEmptyWorkspaceObject(Workspace.getWorkspace());
