...
class SubtractionOperatorWorkspaceObject extends WorkspaceObject
{
   SubtractionOperator op;
   PartSink leftSink, rightSink;

   public SubtractionOperatorWorkspaceObject(Workspace w)
   {
      super(w);
   }
   ...
   static SubtractionOperatorWorkspaceObject load(Node node) throws ProgramLoadingException
   {
      if(!node.getNodeName().equals("subtraction"))
	 throw new ProgramLoadingException();

      SubtractionOperatorWorkspaceObject obj = new SubtractionOperatorWorkspaceObject(Workspace.getWorkspace());
